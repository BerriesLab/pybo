...
class FormACO(MCMultiObjectiveBase):
    ...
    def __init__(self, device: torch.device, dtype: torch.dtype):
        super().__init__(
            ...
            dtype=dtype,
            par_cfg=[
                ParCfg(label="i_max", bounds=(7.5, 15.0)),
                ParCfg(label="i_p", bounds=(3.0, 7.5)),
                ParCfg(label="t_max", bounds=(0.1 * self._t_on, self._t_on))
            ],
            obj_cfg=[
                ObjCfg(label="machining time", to_minimize=True, ref_point=360.0, bounds=(0, 350)),
                ObjCfg(label="tool wear", to_minimize=True, ref_point=160.0, bounds=(0, 150)),
