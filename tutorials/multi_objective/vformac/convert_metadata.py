...
# session or EXP_NNN folder they happen to sit in. len(PAR_MAP) is dim: it holds
# regardless of whether the live objective loaded above.
N_INITIAL = args.n_initial if args.n_initial is not None else 5 * (len(PAR_MAP) + 1)
...
for path, meta, values in parsed:
    ...
    # The rig's own "experiment_type" field names the arm ("bayesian", "sobol"); pybo's
