...
class SparkAcceleratorConstrained(MCMultiObjectiveBase):
    ...
    def __init__(self, device: torch.device, dtype: torch.dtype):
        super().__init__(
            ...
            dtype=dtype,
            par_cfg=[
                ParCfg(label="V0 (V)", bounds=(60, 120)),
                ParCfg(label="dV (V)", bounds=(60, 85)),
                ParCfg(label="Delay Time 1 (μs)", bounds=(0.5 * self._t_r, 0.8 * self._t_r)),
                ParCfg(label="Delay Time 2 (μs)", bounds=(1.2 * self._t_r, 1.8 * self._t_r)),
            ],
            obj_cfg=[
